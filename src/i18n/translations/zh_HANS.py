...
error_translations = (
    ...
    ("Render Failed: Scene Camera Not Found, Please add a camera and try again", "场景相机未找到,请添加相机后再尝试",
     None),
...
需要注意的是在编辑模式的顺序为 主图(当前图片),遮罩图片(如果有的话),参考图片""", None),
    ...
    ("Use Development Mode", "开启调试模式", PROP_TCTX),
    ...
    ("Experimental Features", "试验特性", PROP_TCTX),
    ("Enable experimental features that may be unstable, Restart if enabled", "优先使用工具新特性(可能不稳定), 启用后请重启Blender", None),
    ("BLenderAiStudio Update Tips", "BLenderAiStudio 更新提示", ""),
    ("Upload Error Report", "上传错误报告", "Operator"),
    ("Collect logs and upload to server", "收集日志并上传到服务器", ""),
